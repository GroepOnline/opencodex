...
| Claude Desktop 3P library | Laptop Claude Desktop reads `~/.config/Claude-3p` on the laptop, not the proxy host homedir. `GET /api/claude-desktop/3p-library` (management) and `GET /v1/claude-desktop-3p-library` (data-plane `ocx_` auth, same-origin) return the applied library so a local helper can copy it through ocx-tunnel. Both stay authenticated. The JSON can contain `inferenceGatewayApiKey`; never log the body. `POST /api/claude-desktop/apply` also returns `laptopSync: "ocx-desktop://sync"`. |

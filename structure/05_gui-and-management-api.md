...
exact request origin. A non-loopback dashboard uses the management token flow, a verified
Cloudflare Access JWT, or a verified Authentik OIDC identity (`OIDC_ISSUER` +
`OIDC_CLIENT_ID`; browser code flow when `OIDC_CLIENT_SECRET_FILE` is set).
Cloudflare Access remains the live public-host gate until operators execute
`deploy/oidc/CUTOVER-CHECKLIST.md`. Neither Access nor OIDC admits `/v1/*`.
The server issues an in-memory session for five minutes, capped at 128 live sessions. The session is bound to the
...
| Providers                 | Create/update/delete ordinary provider configs and enrich registry metadata. The reserved `openai` card exposes Pool(default)/Direct account mode; `openai-apikey` remains the separate API route. `POST /api/providers/models/refresh?name=` force-fetches one provider's live `/models` catalog (clears TTL and discovery cooldown) and resyncs the Codex catalog. `GET /api/availability` is the live routing read-model (pool size, first hop, cooling keys, cap window; no secrets).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Logs & Debug              | One sidebar entry (`#verkeer`; legacy `#logs` redirects) with two tabs. Logs tab (`#verkeer` or `#verkeer/logs`): request/runtime logs for local diagnosis. Debug tab (`#verkeer/debug`; legacy `#logs/debug` and `#debug` redirect): provider + usage toggles, refresh/follow log viewer. Usage analytics live under `#verbruik` (legacy `#usage` redirects). `GET/PUT /api/debug`; `GET /api/debug/logs` and `GET /api/debug/usage-logs` (monotonic `after` cursor, legacy `since` accepted). CLI: `ocx debug provider                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | usage …` (both streams via running proxy API). |
...
| Claude Desktop 3P library | Laptop Claude Desktop reads `~/.config/Claude-3p` on the laptop, not the proxy host homedir. `GET /api/claude-desktop/3p-library` (management authentication — admin token, GUI session, CF Access, or Authentik OIDC) and `GET /v1/claude-desktop-3p-library` (data-plane admission secret, required even on loopback, same-origin) return the applied library so a local helper can copy it through ocx-tunnel. Both routes set `Cache-Control: no-store` on every response. The JSON can contain `inferenceGatewayApiKey`; never log the body. `POST /api/claude-desktop/apply` also returns `laptopSync: "ocx-desktop://sync"`.                                                                                                                                                                                                                                                                                                                                                            |

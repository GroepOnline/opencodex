...
| Claude Desktop 3P library | Laptop Claude Desktop reads `~/.config/Claude-3p` on the laptop, not the proxy host homedir. `GET /api/claude-desktop/3p-library` (management authentication — admin token, GUI session, or CF Access) and `GET /v1/claude-desktop-3p-library` (data-plane admission secret, required even on loopback, same-origin) return the applied library so a local helper can copy it through ocx-tunnel. Both routes set `Cache-Control: no-store` on every response. The JSON can contain `inferenceGatewayApiKey`; never log the body. `POST /api/claude-desktop/apply` also returns `laptopSync: "ocx-desktop://sync"`. |

...
| Providers | Create/update/delete ordinary provider configs and enrich registry metadata. The reserved `openai` card exposes Pool(default)/Direct account mode; `openai-apikey` remains the separate API route. `POST /api/providers/models/refresh?name=` force-fetches one provider's live `/models` catalog (clears TTL and discovery cooldown) and resyncs the Codex catalog. |

...
import {
  PageHeader,
  PageSubtitle,
} from "../src/components/primitives/page-header";
import {
  PageTab,
  PageTabPanel,
  PageTabs,
...
import {
  ...
  ModalDesc,
  ModalActions,
} from "../src/components/primitives/modal";
...
describe("composed primitive render contracts", () => {
  ...
  test("tabs preserve selection, focus order, and panel relationships", () => {
    const html = renderToStaticMarkup(
      <>
        ...
        <PageTabPanel id="first-panel" labelledBy="first" tabIndex={0}>
          Visible
        </PageTabPanel>
        <PageTabPanel id="second-panel" labelledBy="second" hidden>
          Hidden
        </PageTabPanel>
      </>,
    );
    ...
    expect(html).toContain(
      'aria-selected="true" aria-controls="first-panel" tabindex="0"',
    );
    expect(html).toContain(
      'aria-selected="false" aria-controls="second-panel" tabindex="-1"',
    );
    expect(html).toContain(
      'tabindex="0" role="tabpanel" id="first-panel" aria-labelledby="first"',
    );
    expect(html).toContain(
      'tabindex="0" role="tabpanel" id="second-panel" aria-labelledby="second" hidden=""',
    );
  });

  test("div modal defaults survive undefined props and preserve explicit opt-out", () => {
    expect(renderToStaticMarkup(<Modal aria-modal={undefined} />)).toContain(
      'aria-modal="true"',
    );
    expect(renderToStaticMarkup(<Modal aria-modal={false} />)).toContain(
      'aria-modal="false"',
    );
    expect(renderToStaticMarkup(<Modal role="alertdialog" />)).toContain(
      'role="alertdialog"',
    );
  });
  ...
  test("native modal composition keeps dialog semantics and accessible text wiring", () => {
    ...
    expect(html).toContain(
      '<p class="modal-desc" id="dialog-description">Description</p>',
    );
  ...
  test.each([
    ...
    "progress %s/%s clamps visual and accessible values together",
    (value, max, now, safeMax, width) => {
      const html = renderToStaticMarkup(
        <ProgressTrack value={Number(value)} max={Number(max)} label="Usage" />,
      );
      expect(html).toContain('role="progressbar" aria-label="Usage"');
      expect(html).toContain(
        `aria-valuemax="${safeMax}" aria-valuenow="${now}"`,
      );
  ...
  test("timestamp emits machine-readable ISO and localized display", () => {
    ...
    const html = renderToStaticMarkup(
      <Timestamp
        value={value}
        locale="en"
        options={{ timeZone: "UTC", hour12: false }}
      />,
    );
  ...
  test.each([
    ...
    (value) => {
      const html = renderToStaticMarkup(
        <Timestamp value={value} locale="en" />,
      );
      ...
      expect(dated).toContain("<time>");
      expect(dated).not.toContain("dateTime=");
    },
  );
});

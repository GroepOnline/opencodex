...
test("Models provider headers use wrap-safe classes and a sibling toggle boundary", async () => {
  const src = await Bun.file(
    new URL("../src/pages/models-provider-card.tsx", import.meta.url),
  ).text();
  ...
  expect(src).not.toMatch(
    /className="row models-provider-actions"\s+onClick=\{e => e\.stopPropagation\(\)\}/,
  );
  // The shared provider component owns the heading and its sibling actions.
...
test("Models workspace stacks via content-width container query before mobile drawer", async () => {
  const css = await Bun.file(
    new URL("../src/styles-models-workspace.css", import.meta.url),
  ).text();

...
export function Notice({
  ...
  return (
    <div
      className={`notice ${tone === "ok" ? "notice-ok" : "notice-err"}`}
      role="status"
    >
...
export function Select({
  ...
  useLayoutEffect(() => {
    ...
    const nextStyle = computeSelectMenuStyle(
  ...
  const onTriggerKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    ...
    switch (event.key) {
      case "ArrowDown":
        ...
        openAt(
          open ? Math.min(options.length - 1, activeIndex + 1) : selectedIndex,
        );
        break;
      case "ArrowUp":
        event.preventDefault();
  ...
  const dropdown = open ? (
    <div
      ...
      aria-label={label}
      style={
        portal ? { ...menuStyle, zIndex: 60, ...dropdownStyle } : dropdownStyle
      }
    >
      {options.map((o, index) => (
        <button
  ...
  return (
    ...
    >
      ...
      {open && (
        <span
          id={tipId}
          className={`ocx-tooltip-bubble ocx-tooltip-bubble--${side}`}
          role="tooltip"
          style={{ maxWidth }}
        >
          {content}

...
export function CodexAccountPoolCards({
  ...
  const t = useT();
  const isNext = (account: CodexAccountEntry) =>
    !account.paused && activeId === account.id;
  ...
  return (
    <>
      {pool.map((a) => {
        ...
        const showReauth =
          Boolean(a.needsReauth) || oauthHealthShowsReauth(healthStatus);
        ...
        return (
          <div
            key={a.id}
            className={`card ${isNext(a) ? "card-active" : ""}`}
            style={{ marginBottom: 8 }}
          >
            <div className="card-head">
              <span
                className={`dot ${showReauth ? "dot-amber" : isNext(a) ? "dot-blue" : "dot-muted"}`}
              />
            ...
            <div className="card-sub">
              {a.email}
              {a.plan ? ` · ${a.plan}` : ""} · {t("prov.accountId")}:{" "}
              {displayAccountId(a.id)}
            </div>
            {healthSummary && (
...
export function CodexAccountPoolReauthBanner({
  ...
  return (
    ...
    >
      <span>
        <IconAlert width={15} /> {t("codexAuth.tokenExpired")}
      </span>
      <button
        type="button"
        className="btn btn-primary btn-sm"

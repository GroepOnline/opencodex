...
} from "./codex-account-pool-utils";

export function CodexCreditItem({
  index,
  grantedAt,
  expiresAt,
  isNext,
  ...
  return (
    <div className={`credit-item${isNext ? " credit-next" : ""}`}>
      <div className="credit-item-head">
        ...
        <span className="credit-item-label">
          {isNext
            ...
            : t("codexAuth.creditLabel", { n: String(index + 1) })}
        </span>
        {isNext && (
          <span
            className="badge badge-amber text-micro"
      ...
      <div className="credit-item-dates">
        <span>
          {t("codexAuth.creditGranted", {
            date: formatCreditDate(grantedAt, locale),
          })}
        </span>
        <span className={urgent ? "credit-urgent" : ""}>
          {t("codexAuth.creditExpires", {

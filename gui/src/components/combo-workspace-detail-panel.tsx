...
export function DetailPanel({
  ...
  const updateDraft = useCallback(
    (updater: (prev: ComboItem) => ComboItem) => {
  ...
  const save = async () => {
    ...
    const renameFrom =
      !isCreate && trimmedId !== baseline.id ? baseline.id : undefined;
    try {
      ...
      setMsg({
        ok: true,
        text: isCreate
          ? t("cws.created", { model: item.model })
          : t("cws.saved"),
      });
      onSaved(item);
    } finally {
  ...
  return (
    <div className="combos-workspace-detail">
      <div className="combos-workspace-detail-head">
        ...
        <div className="combos-workspace-detail-actions">
          ...
          >
            {busy
              ? t("common.saving")
              : t(isCreate ? "cws.create" : "common.save")}
          </button>
        </div>
      </div>
      ...
      <PageTabs label={t("nav.combos")} className="combos-workspace-tabs">
        <PageTab
          id="cws-tab-config"
          controls="cws-panel-config"
          selected={tab === "config"}
        ...
        <PageTab
          id="cws-tab-about"
          controls="cws-panel-about"
          selected={tab === "about"}
          className={`combos-workspace-tab${tab === "about" ? " combos-workspace-tab--active" : ""}`}
            ...
            <div className="cwi-field">
              ...
              >
                {isCreate
                  ...
                  : t("cws.field.idHintEdit", {
                      model: comboPublicModelId(draft.id, draft.alias),
                    })}
              </p>
            </div>
            <div className="cwi-field">

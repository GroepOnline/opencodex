...
export default function ClaudeCode({ apiBase }: { apiBase: string }) {
  ...
  const [state, setState] = useState<ClaudeCodeState | null>(
    () => seedClaudeCode(cacheKey)?.state ?? null,
  );
  const [rows, setRows] = useState<MapRow[]>(
    () => seedClaudeCode(cacheKey)?.rows ?? [],
  );
  const [status, setStatus] = useState("");
  const [ok, setOk] = useState(false);
  const [loading, setLoading] = useState(
    () => !seedClaudeCode(cacheKey)?.state,
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await readJsonOrThrow<
        ClaudeCodeState & { modelMap?: Record<string, string> }
      >(res, t("claude.loadFail"));
      if (!r) {
        setOk(false);
      ...
      const nextState: ClaudeCodeState = {
        ...
        authMode:
          r.authMode === "proxy" || r.authMode === "subscription"
            ? r.authMode
            : "auto",
  ...
  const autoCompactOptions = useMemo(() => {
    const ladder = [
      100_000, 200_000, 250_000, 300_000, 350_000, 400_000, 500_000, 600_000,
    ...
    const values =
      current !== null && !ladder.includes(current)
    ...
    return [
      ...
      ...values.map((value) => ({
        value: String(value),
        label: formatCompactWindow(value, localeTag),
      })),
    ];
  ...
  const save = async () => {
    ...
    for (const row of rows) {
      if (row.from.trim() && row.to.trim())
        modelMap[row.from.trim()] = row.to.trim();
    }
    try {
  ...
  if (!state)
    return <Notice tone="err">{status || t("claude.loadFail")}</Notice>;
  ...
  return (
    <div className="claudecode-workspace-shell ocx-page-root">
      ...
      />
      <PageSubtitle>{t("claude.subtitle")}</PageSubtitle>
      ...
      <div className="claudecode-workspace-root">
        ...
        >
          <div className="claudecode-workspace-rail-list ocx-reveal-list">
            {sections.map((s) => (
              ...
              >
                <span className="claudecode-workspace-rail-name">
                  {s.label}
                </span>
              </button>
            ))}
        ...
        <section
          className="claudecode-workspace-main"

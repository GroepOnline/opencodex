 ...
 */
const FAMILY_COLLAPSE = makeCollapseStore(
...
function cloneProfile(profile: DesktopProfile): DesktopProfile {
  return {
    ...
    ...(profile.appliedFingerprint !== undefined
      ? { appliedFingerprint: profile.appliedFingerprint }
      : {}),
    ...(profile.appliedAt !== undefined
...
function seedDesktop(cacheKey: string) {
  ...
  return {
    ...
    destinations: cached?.data
      ? Object.fromEntries(
          cached.data.models.map((model) => [
            model.route,
            cached.profile.assignments[model.route]?.family ?? "opus",
          ]),
        )
    ...
    status.activeProfile === false
      ...
      : status.stale
        ...
        : status.applied
          ...
          : notAppliedLabel;
  return (
    <div className={`claude-status-bar ${tone}`}>
      ...
      {status.health.requestCount > 0 && (
        <span className="claude-status-health">{statsLabel}</span>
      )}
    </div>
  );
}

function ClaudeLaneBody({ id, children }: { id: string; children: ReactNode }) {
...
export default function ClaudeDesktop({
  ...
  const [profile, setProfile] = useState<DesktopProfile | null>(
    () => seedDesktop(cacheKey).profile,
  );
  const [savedProfile, setSavedProfile] = useState<DesktopProfile | null>(
    () => seedDesktop(cacheKey).savedProfile,
  );
  const [destinations, setDestinations] = useState<Record<string, Family>>(
    () => seedDesktop(cacheKey).destinations,
  );
  ...
  const [collapsedFamilies, setCollapsedFamilies] = useState<Set<string>>(
    () => FAMILY_COLLAPSE.read() ?? new Set(FAMILIES),
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const payload = await readJsonOrThrow<
        DesktopResponse | { error?: string }
      >(response, t("claudeDesktop.loadFail"));
      ...
      setDestinations(
        Object.fromEntries(
          payload.models.map((model) => [
            model.route,
            normalized.assignments[model.route]?.family ?? "opus",
          ]),
        ),
      );
      ...
      if (!hasCacheRef.current) {
        setLoadError(
          error instanceof Error ? error.message : t("claudeDesktop.loadFail"),
        );
      }
    } finally {
      setLoading(false);
  ...
  const modelsByFamily = useMemo(() => {
    const grouped = Object.fromEntries(
      FAMILIES.map((family) => [family, [] as DesktopModel[]]),
    ) as Record<Family, DesktopModel[]>;
    if (!data || !profile) return grouped;
    for (const model of data.models)
      grouped[profile.assignments[model.route]?.family ?? "opus"].push(model);
    return grouped;
  }, [data, profile]);
  ...
  const effectiveDefaults = useMemo(() => {
    ...
    for (const family of FAMILIES) {
      const active = modelsByFamily[family]
  ...
  const moveModel = (route: string, family: Family) => {
    ...
    setProfile((current) => {
      ...
      const assignments = {
        ...current.assignments,
        [route]: { ...previous, family },
      };
      const defaults = { ...current.defaults };
      if (defaults[previous.family] === route) {
        defaults[previous.family] =
          Object.keys(assignments)
            .filter(
              (key) =>
                key !== route && assignments[key].family === previous.family,
            )
            .sort()[0] ?? null;
      }
    ...
    setAnnouncement(
      t("claudeDesktop.moved", { route, family: t(FAMILY_KEYS[family]) }),
    );
  };
  ...
  const save = async (applyAfter: boolean) => {
    ...
    try {
      ...
      await readJsonOrThrow<{ error?: string }>(
        response,
        t("claudeDesktop.saveFailed"),
      );
      ...
      if (applyAfter) {
        ...
        const applyResponse = await fetch(
          `${apiBase}/api/claude-desktop/apply`,
          { method: "POST" },
        );
        await readJsonOrThrow<{ error?: string }>(
          applyResponse,
  ...
  const importProfile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const candidate = JSON.parse(
        await file.text(),
      ) as Partial<DesktopProfile>;
      if (
        candidate.version !== 1 ||
        !candidate.assignments ||
        !candidate.defaults
      )
        throw new Error(t("claudeDesktop.importExpected"));
      const imported = normalizeProfile({
        ...data!,
        profile: candidate as DesktopProfile,
      });
      setProfile(imported);
  ...
  return (
    <>
      <PageHeader
        ...
        description={t("claudeDesktop.subtitle", { port: data.port })}
        descriptionClassName="page-sub"
        actionsClassName="claude-profile-tools"
        actions={
          <>
            <input
              ref={importRef}
      ...
      {status && (
        <ClaudeDesktopStatusBar
          ...
          statsLabel={t("claudeDesktop.health.stats", {
            ...
            errors: status.health.errorCount,
          })}
          appliedLabel={t("claudeDesktop.status.applied")}
          notAppliedLabel={t("claudeDesktop.status.notApplied")}
          staleLabel={t("claudeDesktop.status.stale")}
          notActiveLabel={t("claudeDesktop.status.notActiveProfile")}
        />
      )}
      ...
      >
        ...
        <a
          className={`btn btn-ghost${pending !== null ? " disabled" : ""}`}
          href={pending === null ? LAPTOP_SYNC_HREF : undefined}
          ...
          onClick={(event) => {
            ...
            setAnnouncement(t("claudeDesktop.syncLaptopAnnounce"));
          }}
        >

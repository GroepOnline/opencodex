...
export function DebugClaudeInboundPanel({
  entries,
  feedback,
}: {
  ...
  return (
    <div className="card" style={{ marginBottom: 16, padding: "12px 14px" }}>
      ...
      <div className="muted text-control" style={{ marginBottom: 10 }}>
              ...
              {entries.map((entry) => (
                <tr key={entry.id}>
                  <td className="muted mono">
                    {formatClaudeInboundTime(entry.at)}
                  </td>
                  ...
                  <td className="mono" title={entry.metadataKeys?.join(", ")}>
                    {entry.hasMetadataUserId
                      ? `user_id ${entry.userIdTag ?? ""}`
                      : t("debug.claudeInbound.none")}
                  </td>
                  <td className="mono">
                    {entry.hasSystem
                      ? (entry.systemTag ?? "yes")

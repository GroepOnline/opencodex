...
export function parseDebugLogQuery(url: URL): { after: number; limit: number } {
  const after = Number(
    url.searchParams.get("after") ?? url.searchParams.get("since") ?? "0",
  );
...
export function tokPerSecondResult(
  entry: Pick<MetricSource, "durationMs" | "usageStatus" | "usage">,
): TokPerSecondResult {
  ...
  if (value === null) {
    return {
      kind: "unavailable",
      reason:
        entry.usage.outputTokens <= 0 ? "output_missing" : "invalid_duration",
    };
  }
  return {
    kind: "value",
    value,
    estimated:
      entry.usageStatus === "estimated" || entry.usage.estimated === true,
  };
}

export function unavailableCostReason(
  entry: MetricSource,
  ...
  if (!normalizeCostTokens(entry.usage)) {
    const effectiveRead =
      entry.usage.cacheReadInputTokens ?? entry.usage.cachedInputTokens ?? 0;
...
export function costResult(entry: MetricSource): CostResult {
  ...
  if (!estimate)
    return { kind: "unavailable", reason: unavailableCostReason(entry) };
  const estimateReasons = [
    entry.usageStatus === "estimated" || entry.usage?.estimated
      ? ("usage_estimated" as const)
      : undefined,
    entry.usage &&
    entry.usage.cachedInputTokens === undefined &&
    entry.usage.cacheReadInputTokens === undefined &&
    entry.usage.cacheCreationInputTokens === undefined
      ? ("cache_detail_missing" as const)
      : undefined,
    estimate.price?.source === "expected" ||
    estimate.attempts?.some((a) => a.price.source === "expected")
      ? ("expected_price_overlay" as const)
      : undefined,
 ...
 */
export async function fetchGrokCandidateModels(
  config: OcxConfig,
): Promise<GrokCandidateModel[]> {
  const {
    filterCatalogVisibleModels,
    nativeOpenAiContextWindow,
    visibleNativeSlugs,
  } = await import("../../codex/catalog");
  const routed = filterCatalogVisibleModels(
...
export function stripRegistryOnlyStaticHeaders(
  name: string,
  provider: OcxProviderConfig,
): OcxProviderConfig {
  const entry = providerMatchesRegistryTransport(name, provider)
  ...
  const matchesRegistryStaticHeaders = staticEntries.every(
    ([key, value]) => provider.headers?.[key] === value,
  );
...
export async function buildClaudeDesktopState(
  config: OcxConfig,
  stored?: OcxClaudeDesktopProfile,
) {
  const {
    filterCatalogVisibleModels,
    nativeOpenAiContextWindow,
  ...
  const { DESKTOP_SUPPORTS_1M_THRESHOLD } =
    await import("../../claude/desktop-3p");
  const { parseDesktopProfile, reconcileDesktopProfile, renderDesktopProfile } =
  ...
  // First run (no stored profile) still seeds from the visible catalog, but a
  // saved profile is operator-owned. Re-reconciling the whole catalog on every
  // GET filled all 365 Claude Desktop date aliases and then 400'd new models.
  const raw = stored ?? config.claudeCode?.desktopProfile;
  const profile =
    raw == null
      ? reconcileDesktopProfile(undefined, profileModels)
      : parseDesktopProfile(raw);
  const available = new Set(profileModels.map((model) => model.route));
  const modelByRoute = new Map(
    profileModels.map((model) => [model.route, model]),
  );
  ...
  const models = Object.keys(profile.assignments)
    .sort()
    .map((route) => ({
      route,
      label: modelByRoute.get(route)?.label ?? route,
      available: available.has(route),
      ...(modelByRoute.get(route)?.contextWindow
        ? { contextWindow: modelByRoute.get(route)!.contextWindow }
        : {}),
      ...
      supports1m:
        (modelByRoute.get(route)?.contextWindow ?? 0) >=

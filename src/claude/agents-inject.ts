...
export function buildClaudeAgentDefs(config: OcxConfig, windows: Record<string, number>, configDir = claudeConfigDir()): ClaudeAgentDef[] {
  ...
  if (config.claudeCode?.agentRouting === "dynamic") {
    return [{
      ...
      model: "haiku",
      routeMode: "dynamic",
      description: "Delegate work through opencodex dynamic routing. OCX selects an available backend from the configured featured roster for each dispatch and applies bounded failover.",
...
function renderAgentDef(def: ClaudeAgentDef): string {
  ...
  return [
    ...
    ...(def.routeMode ? [`<!-- ocx-route-mode: ${def.routeMode} -->`] : [`<!-- ocx-route: ${def.route} -->`]),
    ...
    ...(def.dynamic
      ? ["manual edits will be overwritten; disable dynamic routing to remove it."]

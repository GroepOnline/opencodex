# Changelog

All notable changes to the GroepOnline `opencodex` fork. Format follows
[Keep a Changelog](https://keepachangelog.com/) and the project uses
[SemVer](VERSIONING.md). Released GitHub Release bodies are generated by
`scripts/release-notes.ts`; this file is the durable record.

## [1.5.0] — 2026-09-15

### Added

- Unified GUI system layer: one page/panel/tab/modal primitive set across Dashboard,
  Usage, Traffic, Logs, Debug, System, Providers, Claude, Claude Code/Desktop, Grok,
  Models, Combos, Subagents, Storage and API keys, with segmented sub-navigation,
  travelling indicators that stay accessible under keyboard and reduced motion, modal
  focus management and localized Verkeer/startup readings (#238).
- Dynamic Claude agent routing: generated agents route through policy-preserving
  dynamic targets, with disabled routes filtered and routing mode distinguished (#239).

### Fixed

- Combo routes check renames before the generic id guard so rename attempts get the
  specific 403; request-local dynamic Claude agent combos are validated (#243).
- Public (unauthenticated) build provenance keeps `schema_version` null; authenticated
  provenance keeps the real schema value (#243).

### Changed

- Version 1.5.0 is the first release since 1.4.2 (2026-09-08); the live proxy on
  chef-control-az-01 stays on 1.4.2 until the coordinated deploy of this tag.

## [1.4.2] — 2026-09-08

### Fixed

- Storage diagnostics run outside the proxy request thread, keeping health checks
  and other requests responsive while large session archives are scanned.
- Fleet deployment constrains service-token file permissions to the resolved
  container identity and verifies readability before starting or rolling back.

### Changed

- npm and GHCR publication require successful full-platform CI for the exact
  release commit. Release verification and the container runtime use the Bun
  version bundled in the package.

## [1.4.1] — 2026-09-08

### Fixed

- Remote-client launchers handle macOS physical temporary paths while retaining
  native-home and symlink containment checks.
- Cross-platform release and launcher fixtures use bounded, portable execution;
  Windows test shards report all batches without hiding earlier failures.

### Changed

- Release preflight verifies Linux, macOS and Windows before publication.

## [1.4.0] — 2026-09-08

### Changed

- Integrated the reviewed dashboard, tool-wire handling and remote-client artifact
  updates through PRs #193, #195 and #197.
- External review is advisory; executing maintainers retain exact-head technical
  verification and explicit security analysis before merge and release.

## [1.2.1] — 2026-08-18

### Fixed

- Live: Claude Desktop laptop-sync, 3P library endpoints, Dutch provider i18n, and systemd system-unit diagnose rescued from the chef-control-01 snowflake (#66, GRO-1375).
- Live checkout: `diagnoseLiveCheckout` on doctor and startup-health (sha, branch, detached, dirty; no diff text). `scripts/assert-live-checkout-safe.sh` refuses dirty porcelain and a HEAD that is not an ancestor of the deploy target.

### Changed

- Package and `/healthz` version now match the live hotfix revision (`03916957` and descendants). Do not redeploy tag `v1.2.0` (`15eb72dbe`): that commit predates #66 and would wipe laptop-sync.

## [1.2.0] — 2026-08-14

### Security

- GUI: `nanoid` override `>=3.3.18` clears GHSA-2v37-7h3g-55p8 (high, transitive via `vite › postcss`) so `bun audit` passes again (#54).

### CI

- `service-lifecycle.yml`: removed the dead `if: false` macos-launchd/windows-schtasks jobs that carried duplicate `runs-on` YAML keys (actionlint red) (#54); `tests/ci-workflows.test.ts` now pins the linux-only reality.

### Fixed

- Catalog: parse Google AI Studio live model lists (`models[].name`) instead of treating every 2xx as malformed.
- Catalog: registry fallback for context windows/modalities restores `google-combo` when config omitted per-model metadata.
- Providers: `google-antigravity` skips Cloud Code Assist `/models` (no more startup HTTP 404 spam); DeepSeek seed drops deprecated `deepseek-chat`/`deepseek-reasoner`.
- Management API: `GET /api/providers` reports registry-enriched `liveModels` so the GUI matches catalog gather.
- Claude: gateway model-cache refresh authenticates to tunnelled proxies; host-managed stays on with an admission token so `/login` does not clash.
- GUI: `#dashboard` is the landing route again (no longer rewritten to `#leveranciers`).
- Claude: the `/v1/models` admission token skips opencodex-owned dummy markers, so a configured key or data-plane service token is no longer masked on a tunnelled proxy.
- Claude: a journaled owned marker is deleted from `settings.env` instead of being restored as the user's prior token.
- Catalog: model ids are re-validated after the discovery prefix strip, so `models/ gemini-3.5-flash` no longer becomes the id `" gemini-3.5-flash"`.
- GUI: the dashboard uptime card uses the locale-aware formatter (Dutch `u` for hours) instead of hardcoded English units.

### Changed

- GUI: removed the unused `@gitlawb/openclaude` dependency.
- GUI: dropped emoji from warning copy (`⚠`) to match the design-language ban; removed the
  unreferenced `dash.shadowCallWarning` key.

## [1.1.1] — 2026-08-09

Published `@groeponline/opencodex@1.1.1` (npm `latest`) and Homebrew `opencodex` 1.1.1.

### Added

- Tag-driven npm publish workflow (`publish-on-tag.yml`) with OIDC / `NPM_TOKEN` fallback.
- ClinePass provider restore on the fork release train.

### Fixed

- CI path-filter contract includes the publish-on-tag workflow.
- Verkeer route wired into the app hash map.

## [1.0.0] — 2026-08-07

First GroepOnline release of the fork. See
[`GROEPONLINE-GITHUB-LINEAR-PLAN.md`](docs/GROEPONLINE-GITHUB-LINEAR-PLAN.md) and the release
commit for the full record.

### Added

- Independent identity: package renamed to `@groeponline/opencodex`, GroepOnline org references,
  Homebrew tap, and release automation wired for the fork.
- Cross-platform package smoke in CI (Linux/macOS/Windows) plus the Node-only global-install
  smoke path.
- Infra lanes on `dev`: plugins contract, Prometheus metrics, admission rate-limit,
  ChefVault provider security.

### Changed

- GUI redesign to the ChefGroep design language (design-system v2): warm off-white surfaces,
  single blue accent, self-hosted General Sans + JetBrains Mono, Lucide icons, four-view IA with
  topbar shell, ripple motion instead of spinners, `devin`/`strak` skins.
- Per-provider quota slices on the management API with a fan-out controller and VERS/VEROUDERD
  stamps in the dashboard.
- Codex request pacer ported to `dev` (`dev` ↔ `main` reconcile).

### Fixed

- GUI test isolation: leaked globals can no longer cross test files.
- Motion contract: ripples stop, static skeletons, real spinners removed.

## [0.x] — upstream history

Versions before the fork tracked upstream `opencodex` (historical source
[lidge-jun/opencodex](https://github.com/lidge-jun/opencodex)). Fork-specific work starts at
`1.0.0`; earlier tags are upstream lineage and are not restated here.

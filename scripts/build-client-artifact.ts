...
export const CODEX_CLIENT_SHIM = [
  ...
  'case "${1:-}" in',
  // Server commands can issue model requests immediately; even daemon start needs a live route.
  "  agents|apply|cloud|completion|doctor|features|help|login|logout|mcp-server|plugin|update|--help|-h|--version|-V|debug) ;;",
  "  *)",
...
export const CODEX_CLIENT_POWERSHELL_SHIM = [
  ...
  "# Server commands may issue model requests immediately; require a live remote proxy first.",
  "$skipEnsure = @('agents', 'apply', 'cloud', 'completion', 'doctor', 'features', 'help', 'login', 'logout', 'mcp-server', 'plugin', 'update', '--help', '-h', '--version', '-V', 'debug') -contains ($args | Select-Object -First 1)",

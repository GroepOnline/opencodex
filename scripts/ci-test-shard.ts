...
export function parseInteger(value: string | undefined, name: string): number {
  ...
  if (!Number.isInteger(parsed))
    throw new Error(`${name} must be an integer; got ${value ?? "<missing>"}`);
  return parsed;
}
...
export function parseShardSelection(
  ...
  if (shardIndex < 0 || shardIndex >= shardCount) {
    throw new Error(
      `shardIndex must be between 0 and ${shardCount - 1}; got ${shardIndex}`,
    );
  }
...
export function assignBalancedShards(
  ...
  for (const shard of shards)
    shard.sort((left, right) => left.path.localeCompare(right.path));
  return shards;
}
...
export async function runAllBatches(
  paths: string[],
  execute: (batch: string[]) => Promise<number>,
): Promise<number> {
  let result = 0;
  for (let offset = 0; offset < paths.length; offset += BATCH_SIZE) {
    const batch = paths.slice(offset, offset + BATCH_SIZE);
    console.log(
      `[ci-test-shard] batch ${Math.floor(offset / BATCH_SIZE) + 1}: ${batch.length} files`,
    );
...
async function main(): Promise<void> {
  const { shardIndex, shardCount } = parseShardSelection(
    Bun.argv[2],
    Bun.argv[3],
  );
  ...
  if (files.length === 0)
    throw new Error("No Bun test files found under tests/");
  ...
  try {
    process.exitCode = await runAllBatches(
      selected.map((file) => file.path),
      (batch) => runBatch(batch, isolated.env),
    );
  } finally {
    isolated.cleanup();
  }

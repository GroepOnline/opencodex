...
describe("provider registry parity", () => {
  ...
  test("key-login export is derived from the registry", () => {
    ...
    expect(KEY_LOGIN_PROVIDERS["opencode-go"].noVisionModels).not.toContain(
      "kimi-k2.7-code",
    );
    ...
    expect(KEY_LOGIN_PROVIDERS["opencode-go"].noTemperatureModels).toContain(
      "kimi-k3",
    );
    expect(KEY_LOGIN_PROVIDERS["opencode-go"].noTopPModels).toContain(
      "kimi-k3",
    );
    expect(KEY_LOGIN_PROVIDERS["opencode-go"].noPenaltyModels).toContain(
      "kimi-k3",
    );
    expect(
      KEY_LOGIN_PROVIDERS["opencode-go"].preserveReasoningContentModels,
    ).toContain("kimi-k3");
    expect(KEY_LOGIN_PROVIDERS.umans.modelContextWindows?.["umans-coder"]).toBe(
      262_144,
    );
    expect(
      KEY_LOGIN_PROVIDERS.umans.modelContextWindows?.["umans-glm-5.2"],
    ).toBe(405_504);
    expect(
      KEY_LOGIN_PROVIDERS.umans.modelInputModalities?.["umans-coder"],
    ).toEqual(["text", "image"]);
    expect(
      KEY_LOGIN_PROVIDERS.umans.modelInputModalities?.["umans-glm-5.2"],
    ).toEqual(["text"]);
    expect(KEY_LOGIN_PROVIDERS["openai-apikey"].models).toEqual([
      "gpt-5.5",
      "gpt-5.6",
      "gpt-5.6-sol",
      "gpt-5.6-terra",
    ...
    expect(
      KEY_LOGIN_PROVIDERS["openai-apikey"].modelContextWindows?.["gpt-5.6-sol"],
    ).toBe(1_050_000);
    expect(
      KEY_LOGIN_PROVIDERS["openai-apikey"].modelContextWindows?.[
        "gpt-5.6-terra"
      ],
    ).toBe(1_050_000);
    expect(
    ...
    expect(
      KEY_LOGIN_PROVIDERS["openai-apikey"].modelMaxInputTokens?.["gpt-5.6-sol"],
    ).toBe(922_000);
    expect(
      KEY_LOGIN_PROVIDERS["openai-apikey"].modelInputModalities?.["gpt-5.5"],
    ).toEqual(["text", "image"]);
    expect(
      (
        KEY_LOGIN_PROVIDERS["openai-apikey"] as unknown as {
          virtualModels?: unknown;
        }
      ).virtualModels,
    ).toBeUndefined();
    const apiRegistry = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "openai-apikey",
    )!;
    ...
    expect(derived.modelMaxInputTokens).not.toBe(
      apiRegistry.modelMaxInputTokens,
    );
    expect(KEY_LOGIN_PROVIDERS.openrouter.models).toContain(
    ...
    expect(
      KEY_LOGIN_PROVIDERS.openrouter.modelContextWindows?.[
        "anthropic/claude-sonnet-5"
      ],
    ).toBe(1_000_000);
    expect(
    ...
    expect(
      KEY_LOGIN_PROVIDERS.deepseek.modelReasoningEfforts?.["deepseek-v4-pro"],
    ).toEqual(["high", "xhigh", "max"]);
    expect(
      KEY_LOGIN_PROVIDERS.deepseek.modelReasoningEffortMap?.["deepseek-v4-pro"]
        ?.xhigh,
    ).toBe("max");
    expect(
      KEY_LOGIN_PROVIDERS.deepseek.modelReasoningEffortMap?.["deepseek-v4-pro"]
    ...
    expect(KEY_LOGIN_PROVIDERS.deepseek.models).toEqual([
      "deepseek-v4-pro",
      "deepseek-v4-flash",
    ]);
  ...
  test("OpenAI API route max-input metadata is trusted and user values only lower it", () => {
    ...
    expect(
      routeModel(makeConfig(1_000_000), "openai-apikey/gpt-5.6-sol").provider
        .modelMaxInputTokens?.["gpt-5.6-sol"],
    ).toBe(922_000);
    expect(
      routeModel(makeConfig(300_000), "openai-apikey/gpt-5.6-sol").provider
        .modelMaxInputTokens?.["gpt-5.6-sol"],
    ).toBe(300_000);
    expect(
      routeModel(makeConfig(922_000), "openai-apikey/gpt-5.6-sol").provider
        .modelContextWindows?.["gpt-5.6-sol"],
    ).toBe(1_050_000);
    expect(
      routeModel(makeConfig(922_000, 350_000), "openai-apikey/gpt-5.6-sol")
        .provider.modelContextWindows?.["gpt-5.6-sol"],
    ).toBe(350_000);
    expect(
      (
        routeModel(makeConfig(300_000), "openai-apikey/gpt-5.6-sol")
          .provider as unknown as { virtualModels?: unknown }
      ).virtualModels,
    ).toBeUndefined();
  });

  test("non-API route max-input metadata keeps user overrides and fills registry defaults", () => {
    const registryEntry = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "zai",
    )!;
    ...
    try {
      ...
      expect(
        routeModel(config, "zai/glm-5.2").provider.modelMaxInputTokens,
      ).toEqual({
        "glm-5.2": 200_000,
        "glm-5.2[1m]": 800_000,
      });
    } finally {
      if (originalMaxInputTokens === undefined)
        delete registryEntry.modelMaxInputTokens;
  ...
  test("registry output-token defaults hydrate stale provider configs and keep user overrides", () => {
    ...
    try {
      ...
      expect(
        providerConfigSeed(registryEntry).modelMaxOutputTokens?.["glm-5.2"],
      ).toBe(128_000);
      expect(deriveKeyLoginMap().zai.modelMaxOutputTokens?.["glm-5.2"]).toBe(
        128_000,
      );
    } finally {
      if (originalDefaultMaxOutputTokens === undefined)
        delete registryEntry.defaultMaxOutputTokens;
      else
        registryEntry.defaultMaxOutputTokens = originalDefaultMaxOutputTokens;
      if (originalModelMaxOutputTokens === undefined)
        delete registryEntry.modelMaxOutputTokens;
  ...
  test("CN provider defaults and context windows match the audited registry refresh", () => {
    ...
    for (const providerId of ["minimax", "minimax-cn"]) {
      ...
      expect(entry?.modelReasoningEfforts?.["MiniMax-M3"]).toEqual([
        "low",
        "medium",
        "high",
        "xhigh",
      ...
      expect(entry?.modelDefaultReasoningEfforts?.["MiniMax-M3"]).toBe(
        "medium",
      );
      expect(entry?.modelReasoningEffortMap?.["MiniMax-M3"]).toMatchObject({
        low: "disabled",
        medium: "adaptive",
        high: "adaptive",
      });
  ...
  test("Alibaba Token Plan exposes the official Beijing model contract separately from Coding Plan", () => {
    ...
    expect(
      KEY_LOGIN_PROVIDERS["alibaba-token-plan"].thinkingBudgetModels,
  ...
  test("aggregator defaults and Neuralwatt seeds match the audited live catalogs", () => {
    ...
    const neuralwatt = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "neuralwatt",
    );
    ...
    expect(neuralwatt?.modelReasoningEfforts?.["glm-5.2-short"]).toEqual(
      neuralwatt?.modelReasoningEfforts?.["glm-5.2"],
    );
    expect(neuralwatt?.modelReasoningEfforts?.["glm-5.2-short-fast"]).toEqual(
      [],
    );
    expect(neuralwatt?.modelReasoningEfforts).not.toHaveProperty(
    ...
    expect(neuralwatt?.preserveReasoningContentModels).toContain(
      "glm-5.2-short",
    );
    expect(neuralwatt?.preserveReasoningContentModels).not.toContain(
      "moonshotai/Kimi-K2.5",
    );
  });
  ...
  test("Z.AI and Kimi context aliases route with bracket-suffix stripping", () => {
    ...
    const optedInProviders = PROVIDER_REGISTRY.filter(
      (entry) => entry.modelSuffixBracketStrip,
    ).map((entry) => entry.id);
    ...
    expect(
      routeModel(config, "zai/glm-5.2[1m]").provider.modelSuffixBracketStrip,
    ).toBe(true);
  });

  test("Anthropic API-key provider mirrors the OAuth entry's models on the key flow", () => {
    const anthropicOauth = PROVIDER_REGISTRY.find(
    ...
    expect(KEY_LOGIN_PROVIDERS["anthropic-apikey"].models).toEqual(
      anthropicOauth?.models,
    );
    expect(KEY_LOGIN_PROVIDERS["anthropic-apikey"].modelContextWindows).toEqual(
      anthropicOauth?.modelContextWindows,
    );
  });
  ...
  test("Kimi coding aliases preserve model context and capability parity", () => {
    ...
    for (const providerId of ["kimi", "kimi-code"]) {
      ...
      for (const modelId of codingModels) {
        expect(entry?.modelContextWindows?.[modelId]).toBe(
          modelId === "k3[1m]" ? 1_048_576 : 262_144,
        );
      }
      ...
      expect(entry?.modelReasoningEfforts?.["k3[1m]"]).toEqual([
        "low",
        "high",
        "max",
      ]);
      ...
      expect(entry?.modelInputModalities?.["k3[1m]"]).toEqual([
        "text",
        "image",
      ]);
    ...
    const kimiModel = applyProviderConfigHints(
      "kimi",
      providerConfigSeed(kimi),
      { provider: "kimi", id: "k3" },
    );
    const kimiEntry = buildCatalogEntries(
      nativeTemplate(),
      [],
      [kimiModel],
    ).find((entry) => entry.slug === "kimi/k3");
    expect(kimiEntry?.default_reasoning_level).toBe("max");

    const moonshot = PROVIDER_REGISTRY.find(
    ...
    expect(moonshot?.modelInputModalities?.["kimi-k3"]).toEqual([
      "text",
      "image",
    ]);
  ...
  test("LiteLLM is the only registry seed with optional key authentication", () => {
    ...
    const optionalKeyProviders = PROVIDER_REGISTRY.filter(
      (entry) => entry.keyOptional,
    ).map((entry) => entry.id);
  ...
  test("NVIDIA NIM is free-tier priced but still requires an API key", () => {
    ...
    const freeTierProviders = PROVIDER_REGISTRY.filter(
      (entry) => entry.freeTier,
    ).map((entry) => entry.id);
  ...
  test("Cloudflare Workers AI GLM 5.2 stays on the host 262k cap", () => {
    const cf = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "cloudflare-workers-ai",
    );
  ...
  test("freeTier propagates through config seed, enrich backfill, and presets without overwriting user config", async () => {
    const { enrichProviderFromRegistry } =
      await import("../src/providers/derive");
    const nvidia = PROVIDER_REGISTRY.find((entry) => entry.id === "nvidia")!;
    ...
    expect(
      deriveProviderPresets().find((p) => p.id === "venice"),
    ).not.toHaveProperty("freeTier");
  });

  test("base URL override permission is registry-only and limited to opted-in providers", () => {
    const optedIn = PROVIDER_REGISTRY.filter(
      (entry) => entry.allowBaseUrlOverride,
    );
    ...
    for (const entry of optedIn) {
      expect(providerConfigSeed(entry)).not.toHaveProperty(
  ...
  test("Ollama Cloud uses the three live tagged IDs without retaining bare aliases", () => {
    const ollamaCloud = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "ollama-cloud",
    );
  ...
  test("CLI init providers are derived from the registry", () => {
    ...
    expect(
      buildInitProviders().find((p) => p.id === "azure-openai")?.adapter,
    ).toBe("azure-openai");
  });
  ...
  test("Cursor registry exposure is dashboard/oauth with live native exec and model discovery", () => {
    ...
    expect(
      deriveProviderPresets().find((preset) => preset.id === "cursor"),
    ).toMatchObject({
      id: "cursor",
      adapter: "cursor",
      auth: "oauth",
    ...
    expect(seed.modelReasoningEfforts?.["gpt-5.5"]).toEqual([
      "low",
      "medium",
      "high",
    ]);
    expect(seed.modelReasoningEfforts?.["gpt-5.6-sol"]).toEqual([
      "low",
      "medium",
      "high",
      "xhigh",
    ...
    const savedCursor: OcxProviderConfig = {
      adapter: "cursor",
      baseUrl: "https://api2.cursor.sh",
    };
    ...
    const initCursor = buildInitProviders().find(
  ...
  test("OAuth provider configs use canonical registry values", () => {
    expect(OAUTH_PROVIDERS.kimi.providerConfig.baseUrl).toBe(
      "https://api.kimi.com/coding/v1",
    );
    expect(OAUTH_PROVIDERS.anthropic.providerConfig.defaultModel).toBe(
      "claude-sonnet-5",
    );
    expect(OAUTH_PROVIDERS.anthropic.providerConfig.models).toContain(
      "claude-sonnet-5",
    );
    expect(OAUTH_PROVIDERS.anthropic.providerConfig.models).toContain(
      "claude-fable-5",
    ...
    expect(
      OAUTH_PROVIDERS.anthropic.providerConfig.modelContextWindows?.[
        "claude-sonnet-5"
      ],
    ).toBe(1_000_000);
    ...
    expect(
      OAUTH_PROVIDERS.xai.providerConfig.modelContextWindows?.["grok-4.5"],
    ).toBe(500_000);
    expect(
      OAUTH_PROVIDERS.xai.providerConfig.modelReasoningEfforts?.["grok-4.5"],
    ).toEqual(["low", "medium", "high"]);
    expect(OAUTH_PROVIDERS.xai.providerConfig.noVisionModels).toContain(
      "grok-build-0.1",
    );
    expect(
    ...
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig.models,
    ...
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig.models,
    ).toContain("gpt-oss-120b-medium");
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig.models,
    ).toContain("gemini-3.1-flash-image");
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig.models,
    ).toHaveLength(6);
    // Effort ladders on collapsed base models.
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelReasoningEfforts?.["gemini-3.6-flash"],
    ).toEqual(["low", "medium", "high"]);
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelReasoningEfforts?.["gemini-3.1-pro"],
    ).toEqual(["low", "high"]);
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelReasoningEfforts?.["claude-opus-4-6-thinking"],
    ).toEqual(["low", "medium", "high", "max"]);
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelReasoningEfforts?.["claude-sonnet-4-6"],
    ).toEqual(["low", "medium", "high", "max"]);
    // Context windows on collapsed base models.
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelContextWindows?.["gemini-3.6-flash"],
    ).toBe(1_048_576);
    expect(
      OAUTH_PROVIDERS["google-antigravity"].providerConfig
        .modelContextWindows?.["gemini-3.1-pro"],
    ).toBe(1_048_576);
  ...
  test("GUI preset projection preserves current featured set plus key catalog and custom", () => {
    ...
    expect(presets.find((p) => p.id === "openai")).toMatchObject({
      label: "OpenAI (Codex login)",
      codexAccountMode: "pool",
    });
    expect(presets.find((p) => p.id === "openai-multi")).toBeUndefined();
    expect(presets.find((p) => p.id === "openai-apikey")?.label).toBe(
      "OpenAI API",
    );
    ...
    expect(presets.find((p) => p.id === "kimi")?.baseUrl).toBe(
      "https://api.kimi.com/coding/v1",
    );
    expect(presets.find((p) => p.id === "anthropic")?.defaultModel).toBe(
      "claude-sonnet-5",
    );
    ...
    expect(presets.find((p) => p.id === "azure-openai")?.adapter).toBe(
      "azure-openai",
    );
    ...
    expect(nextPresets.find((p) => p.id === "openai")!.provider).toEqual(
      providerConfigSeed(
        PROVIDER_REGISTRY.find((entry) => entry.id === "openai")!,
      ),
    );
    expect(
      presets.find((p) => p.id === "openai-apikey")?.provider,
    ).toBeUndefined();
  });
      ...
      [
        {
          ...
          id: "umans-coder",
          contextWindow:
            KEY_LOGIN_PROVIDERS.umans.modelContextWindows?.["umans-coder"],
          inputModalities:
            KEY_LOGIN_PROVIDERS.umans.modelInputModalities?.["umans-coder"],
          reasoningEfforts:
            KEY_LOGIN_PROVIDERS.umans.modelReasoningEfforts?.["umans-coder"],
        },
        {
          provider: "umans",
          id: "umans-glm-5.2",
          ...
          inputModalities:
            KEY_LOGIN_PROVIDERS.umans.modelInputModalities?.["umans-glm-5.2"],
          reasoningEfforts:
            KEY_LOGIN_PROVIDERS.umans.modelReasoningEfforts?.["umans-glm-5.2"],
        },
      ],
    );
  ...
  test("MiniMax metadata lookup tolerates routed lowercase ids", () => {
    expect(
      getJawcodeModelMetadata("minimax", "MiniMax-M2.5")?.contextWindow,
    ).toBe(204_800);
  ...
  test("grok-4.5 flows from the xai registry seed into a built catalog entry (260709 refresh)", () => {
    ...
    const model = applyProviderConfigHints("xai", seed, {
      id: "grok-4.5",
      provider: "xai",
    });
    ...
    expect(
      (entry?.supported_reasoning_levels as { effort: string }[]).map(
        (l) => l.effort,
      ),
  ...
  test("Google combo members get registry context/modalities even when config omitted them", () => {
    ...
    expect(google?.modelContextWindows?.["gemini-3.1-pro-preview"]).toBe(
      1_048_576,
    );
    expect(google?.liveModels).toBe(true);
    const antigravity = PROVIDER_REGISTRY.find(
  ...
  test("the BizRouter preset seeds a usable OpenAI-compatible provider", () => {
    const bizrouter = PROVIDER_REGISTRY.find(
      (entry) => entry.id === "bizrouter",
    );
    ...
    expect(
      entries.find((e) => e.slug === "bizrouter/openai-gpt-5.6-sol"),
    ).toBeTruthy();
  });
});
...
describe("free-provider directory isolation", () => {
  test("directory metadata never becomes a canonical runtime provider", () => {
    ...
    const directoryOnlyIds = FREE_PROVIDER_DIRECTORY.filter(
      (entry) => entry.supportLevel === "reference",
    ).map((entry) => entry.id);
  ...
  test("an id shared by both lists must resolve to the same endpoint", () => {
    ...
    const shared = FREE_PROVIDER_DIRECTORY.filter((row) =>
      registryById.has(row.id),
    );
    expect(shared.length).toBeGreaterThan(0);

    for (const row of shared) {
      const registryEntry = registryById.get(row.id)!;
      expect(`${row.id} -> ${registryEntry.baseUrl}`).toBe(
        `${row.id} -> ${row.baseUrl}`,
      );
    }
  });
  ...
  test("only rows with checked provenance claim a verification date", () => {
    for (const entry of FREE_PROVIDER_DIRECTORY) {
      if (entry.verification === "unverified") {
        ...
        expect(
          entry.documentationUrl ?? entry.modelsUrl ?? entry.dashboardUrl,
        ).toBeTruthy();
      }
    }
  });

...
afterEach(() => {
  ...
  if (previousImagesApiKey === undefined)
    delete process.env.OPENCODEX_TEST_IMAGES_API_KEY;
...
function fakeImagesUpstream(
  ...
  const upstream = Bun.serve({
    port: 0,
    async fetch(req) {
      // Local port-discovery probes are not image relays and have no JSON body.
      // Keep them out of the captured business-request count.
      if (req.method !== "POST")
  ...
  globalThis.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path)
      return originalFetch(new URL(`${path}${url.search}`, upstream.url), init);
...
test("image upstream fixture rejects discovery probes without capturing a relay", async () => {
  const captured: CapturedRequest[] = [];
  const upstream = fakeImagesUpstream(captured);
  try {
    for (const method of ["GET", "HEAD"]) {
      const response = await originalFetch(upstream.url, { method });
...
test("POST /v1/images/generations relays to the ChatGPT forward provider with forwarded auth", async () => {
  ...
  try {
    const response = await fetch(
      new URL("/v1/images/generations", server.url),
      {
        method: "POST",
        headers: {
...
test("an explicit Images provider accepts bearer admission without leaking the proxy secret", async () => {
  ...
  try {
    const response = await fetch(
      `http://127.0.0.1:${server.port}/v1/images/generations`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    expect(
      [...captured[0].headers.values()].some((value) =>
        value.includes("proxy-admission-secret"),
      ),
    ).toBe(false);
  } finally {
    await server.stop(true);
...
test("an invalid explicit Images provider returns 400 after bearer admission", async () => {
  ...
  try {
    ...
    expect(json.error.message).toContain(
      "must be an API-key openai-responses provider",
    );
  } finally {
    await server.stop(true);
  }
...
test.each([
  ...
  "explicit Images provider rejects %s configuration",
  async (_case, provider, expectedError) => {
...
test.each([
  [
    "revoked",
    "chefvault://providers/images/revoked",
    410,
    { code: "revoked", message: "lease revoked" },
    401,
    "authentication_error",
  ],
  [
    "authority_unavailable",
    "chefvault://providers/images/outage",
    503,
  ...
  async (_case, ref, vaultStatus, vaultBody, expectedStatus, expectedType) => {
    ...
    process.env.CHEF_PROVIDER_SECURITY_TOKEN =
      "access-token-images-vault-32chars";
    ...
    try {
      ...
      globalThis.fetch = originalFetch;
      if (previousVaultUrl === undefined)
        delete process.env.CHEF_PROVIDER_SECURITY_URL;
      else process.env.CHEF_PROVIDER_SECURITY_URL = previousVaultUrl;
      if (previousVaultToken === undefined)
...
test("keyed baseUrl with a /v1 suffix is normalized (no double /v1)", async () => {
  ...
  saveConfig({
    ...
    providers: {
      openai: disabledOpenAiProvider,
      "openai-apikey": keyedProvider(
        `${upstream.url.toString().replace(/\/$/, "")}/v1`,
      ),
    },
  } as OcxConfig);
...
test("relays upstream error status and body verbatim", async () => {
  ...
  const upstream = fakeImagesUpstream(captured, 403, {
    error: {
      message: "Your plan does not allow image generation.",
      type: "forbidden",
    },
  });
  saveConfig(forwardConfig(upstream.url.toString().replace(/\/$/, "")));
  ...
  try {
    ...
    expect(json.error.message).toBe(
      "Your plan does not allow image generation.",
    );
  } finally {
    await server.stop(true);
    await upstream.stop(true);
  ...
  globalThis.fetch = ((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (
      ...
      return originalFetch(
        new URL(url.pathname.slice("/backend-api/codex".length), upstream.url),
        init,
      );
    }
    return originalFetch(input, init);
...
test("the proxy admission secret is never relayed to the forward upstream", async () => {
  ...
  try {
    ...
    expect(
      [...captured[0].headers.values()].some((v) => v.includes("local-secret")),
    ).toBe(false);
  } finally {
    await server.stop(true);
    await upstream.stop(true);
...
test("CCA image fallback generates images via Google Antigravity when no OpenAI upstream exists", async () => {
  ...
  try {
    ...
    const body = registryHits[0].body as {
      model?: string;
      request?: { generationConfig?: { responseModalities?: string[] } };
    };
    expect(body.model).toBe("gemini-3.1-flash-image");
    expect(body.request?.generationConfig?.responseModalities).toEqual([
      "TEXT",
      "IMAGE",
    ]);
    expect(registryHits[0].headers.get("authorization")).toBe(
      "Bearer cca-access-token",
    );
    ...
    // OAuth credential.
    expect(registryHits[0].headers.get("user-agent")).toBe(
...
test("keyed/forward relay rejects an oversized streamed body mid-stream with 502", async () => {
  ...
  try {
    ...
    if (previousMax === undefined)
      delete process.env.OPENCODEX_IMAGES_RESPONSE_MAX_BYTES;
...
test("CCA image fallback preserves upstream 400 (not collapsed to 502)", async () => {
  ...
  ccaFetchMock(registryHits, otherHits, {
    status: 400,
    payload: { error: { message: "Invalid prompt content" } },
  });
...
test("CCA image response with non-array parts returns 502 (envelope validation)", async () => {
  ...
  ccaFetchMock(registryHits, otherHits, {
    payload: {
      response: { candidates: [{ content: { parts: "not-an-array" } }] },
    },
  });
...
test("CCA-only request with proxy admission bearer succeeds and never sends it upstream", async () => {
  ...
  try {
    ...
    expect(registryHits).toHaveLength(1);
    expect(
      [...registryHits[0].headers.values()].some((v) =>
        v.includes("proxy-admission-secret"),
      ),
    ).toBe(false);
    expect(registryHits[0].headers.get("authorization")).toBe(
      "Bearer cca-access-token",
...
test("GET /v1/opencodex/artifacts/:id serves opaque artifacts with API auth", async () => {
  process.env.OPENCODEX_API_AUTH_TOKEN = "proxy-admission-secret";
  const { materializeInlineImage, createImageBudget, artifactHttpUrl } =
    await import("../src/images/artifacts");
  const filePath = await materializeInlineImage(
    CCA_TINY_PNG,
    createImageBudget(),
  );
  ...
  try {
    ...
    const traversal = await fetch(
      `http://127.0.0.1:${server.port}/v1/opencodex/artifacts/../package.json`,
      {
        headers: { authorization: "Bearer proxy-admission-secret" },
      },
    );

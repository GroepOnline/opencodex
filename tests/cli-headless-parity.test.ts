...
describe("headless GUI parity CLI", () => {
  test("every GUI management endpoint belongs to a documented CLI resource", () => {
    ...
    const coverage: Array<[string, string]> = [
      ...
      ["/api/v2", "ocx v2/agent"],
      ["/api/whoami", "dashboard session chrome"],
  ...
  test("agent effort and roster use the same live mutation routes as GUI", async () => {
    ...
    expect(
      await handleAgentCommand(
        ["effort", "set", "--main", "high", "--subagent", "medium", "--json"],
        runtime.deps,
      ),
    ).toBe(0);
    expect(
      await handleAgentCommand(
        ["subagents", "set", "a/model,b/model", "--json"],
        runtime.deps,
      ),
    ).toBe(0);
  ...
  test("API key create returns the one-time key through the access command", async () => {
    const runtime = fakeRuntime((req) =>
      new URL(req.url).pathname === "/api/keys"
        ? { id: "key-1", name: "deploy", key: "ocx_secret" }
        : undefined,
    );
    expect(
      await handleAccessCommand(
        ["key", "create", "deploy", "--json"],
        runtime.deps,
      ),
    ).toBe(0);
    expect(runtime.requests[0]).toEqual({
  ...
  test("Grok include edits the persisted exclusion set before apply", async () => {
    ...
    expect(
      await handleGrokCommand(["include", "a", "--json"], runtime.deps),
    ).toBe(0);
    expect(runtime.requests[1]).toEqual({
      path: "/api/grok/selection",
      method: "PUT",
      body: { excluded: ["b"] },
    });
  });
  ...
  test("response cache view and clear use the GUI management routes", async () => {
    const runtime = fakeRuntime();
    expect(await handleObserveCommand(["cache", "--json"], runtime.deps)).toBe(
      0,
    );
    expect(
      await handleObserveCommand(["cache", "clear", "--json"], runtime.deps),
    ).not.toBe(0);
    expect(
  ...
  test("config set validates the complete candidate before the atomic write", async () => {
    ...
    try {
      ...
      expect(
        await handleConfigCommand(["set", "codexAutoStart", "false", "--json"]),
      ).toBe(0);
      expect(
      ...
      expect(
        await handleConfigCommand(["set", "port", "-1", "--json"]),
      ).not.toBe(0);
      expect(
        JSON.parse(readFileSync(join(home, "config.json"), "utf8")).port,
      ).toBe(10100);
    } finally {

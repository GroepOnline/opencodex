...
describe("parseCodexVersionOutput / compareCodexVersions", () => {
  test("parses dotted and prerelease versions", () => {
    ...
    expect(parseCodexVersionOutput("0.145.0-alpha.30")).toBe(
      "0.145.0-alpha.30",
    );
  });
  ...
  test("orders prerelease identifiers numerically", () => {
    ...
    expect(compareCodexVersions("0.145.0", "0.145.0-alpha.30")).toBeGreaterThan(
      0,
    );
    expect(
      compareCodexVersions("0.145.0-alpha.9", "0.145.0-alpha.30"),
    ).toBeLessThan(0);
    expect(
      compareCodexVersions("0.145.0-alpha-1", "0.145.0-alpha-2"),
    ).toBeLessThan(0);
    expect(
      compareCodexVersions("0.145.0-alpha.1.beta", "0.145.0-alpha.1.beta.1"),
    ).toBeLessThan(0);
  });
});
...
describe("resolveCodexRuntime", () => {
  ...
  test("invalid CODEX_CLI_PATH records a diagnostic and continues", () => {
    ...
    expect(result.failures.some((item) => item.source === "environment")).toBe(
      true,
    );
  ...
  test("valid configured runtime beats shim and PATH", () => {
    ...
    writeFileSync(
      join(configDir, "codex-shim.json"),
      JSON.stringify({
  ...
  test("stale shim path is rejected", () => {
    ...
    expect(
      result.failures.some(
        (item) =>
          item.source === "shim" && item.reason.includes("does not exist"),
      ),
    ).toBe(true);
  });
  ...
  test("display path redacts user home segments", () => {
    const shown = displayCodexRuntimePath(
  ...
  test("unrecognized --version output is rejected", () => {
    ...
    expect(
      result.failures.some(
        (item) =>
          item.source === "environment" && item.reason.includes("unrecognized"),
      ),
    ).toBe(true);
    expect(result.runtime.source).not.toBe("environment");
  });
  ...
  test("persists and clears effort clamp diagnostics", () => {
    ...
    expect(
      effortClampAppliesToRuntime(loaded, {
  ...
  test("creates missing config directory on first runtime/clamp persist", () => {
    ...
    expect(loadPersistedCodexRuntime({ configDir })?.command).toBe(
      "C:\\keep\\codex.exe",
    );
  ...
  test("catalog clamp clears diagnostics inside deps.configDir when probe fails", async () => {
    const { clampCatalogModelsToCodexSupport } =
      await import("../src/codex/catalog/effort");
    ...
    expect(loadLastEffortClamp({ configDir: nested })?.removedEfforts).toEqual([
      "max",
    ]);
  ...
  test("persisted runtime stamp busts resolve memo; catalog cache keys by runtime", async () => {
    ...
    const { loadBundledCodexCatalog, resetBundledCatalogCacheForTests } =
    ...
    const oldBin =
      process.platform === "win32"
        ? join(oldDir, "codex.cmd")
        : join(oldDir, "codex");
    const newBin =
      process.platform === "win32"
        ? join(newDir, "codex.cmd")
        : join(newDir, "codex");
    ...
    writeLauncher(newBin, "0.145.0-alpha.30", [
      "low",
      "medium",
      "high",
      "max",
    ...
    try {
      ...
      expect(
        oldCatalog?.models?.[0]?.supported_reasoning_levels?.some(
      ...
      expect(
        newCatalog?.models?.[0]?.supported_reasoning_levels?.some(
          (level) => (level as { effort?: string }).effort === "max",
        ),
      ).toBe(true);
    } finally {
  ...
  test("bundled runtime memo follows CODEX_CLI_PATH changes and self-heals after fallback", async () => {
    ...
    const firstBin =
      process.platform === "win32"
        ? join(firstDir, "codex.cmd")
        : join(firstDir, "codex");
    const secondBin =
      process.platform === "win32"
        ? join(secondDir, "codex.cmd")
        : join(secondDir, "codex");
    ...
    const deps = {
      ...
      fallbackCommand: join(home, "missing-codex"),
    };
    resetCodexRuntimeResolveCacheForTests();
    resetBundledCatalogCacheForTests();

    try {
      const firstCatalog = loadBundledCodexCatalog(deps);
      ...
      runtimeEnv.CODEX_CLI_PATH = secondBin;
      const secondCatalog = loadBundledCodexCatalog(deps);
      ...
      expect(loadBundledCodexCatalog(deps)).toBeNull();
      writeLauncher(secondBin, "0.146.0", "high");
      now += 15_001;
      const recovered = loadBundledCodexCatalog(deps);
  ...
  test("resolve cache throttles fallback probes until TTL, then retries", async () => {
    ...
    try {
      expect(resolveCodexRuntime({ now: clock }).runtime.source).toBe(
        "fallback",
      );

...
describe("error fidelity", () => {
  test("classifyError maps Codex-recognized context/quota/rate failures", () => {
    ...
    expect(
      classifyError(429, "upstream_error", "Rate limit reached for model"),
    ).toMatchObject({
      type: "rate_limit_error",
      code: "rate_limit_exceeded",
    });
    expect(
      classifyError(402, "upstream_error", "You exceeded your current quota"),
    ).toMatchObject({
    ...
    expect(
      classifyError(402, "upstream_error", '{"code":"INFERENCE_CAP_ERROR"}'),
    ).toMatchObject({
      type: "insufficient_quota",
      code: "insufficient_quota",
    });
    expect(
      classifyError(402, "upstream_error", "Payment Required"),
    ).toMatchObject({
      type: "upstream_error",
      code: "upstream_error",
    });
    ...
    expect(
      classifyError(403, "upstream_error", "Provider error 403"),
    ).toMatchObject({
    ...
    expect(
      classifyError(403, "authentication_error", subscription),
    ).toMatchObject({
      type: "authentication_error",
      code: "invalid_api_key",
    });
    ...
    expect(
      classifyError(
        502,
        "upstream_error",
        "Kiro rate limit exceeded: ThrottlingException: rate limited",
      ),
    ).toMatchObject({
      type: "rate_limit_error",
      code: "rate_limit_exceeded",
    ...
    expect(
      classifyError(
        502,
        "upstream_error",
        "Kiro authentication failed: AccessDeniedException: expired token",
      ),
    ).toMatchObject({
      type: "authentication_error",
      code: "invalid_api_key",
    ...
    expect(
      classifyError(
        502,
        "upstream_error",
        "Kiro invalid request: ValidationException: model not found",
      ),
    ).toMatchObject({
      type: "invalid_request_error",
      code: "invalid_request_error",
    ...
    expect(
      classifyError(
        502,
        "upstream_error",
        "Kiro quota exhausted: monthly quota exceeded",
      ),
    ).toMatchObject({
      type: "insufficient_quota",
      code: "insufficient_quota",
  ...
  test("sanitizePassthroughHeaders drops stale and hop-by-hop headers while preserving rate-limit metadata", () => {
    const sanitized = sanitizePassthroughHeaders(
      new Headers({
...
describe("overload and transient-429 classification (F3)", () => {
  test("503 / overloaded maps to the Codex-recognized server_is_overloaded", () => {
    expect(
      classifyError(503, "upstream_error", "The server is overloaded"),
    ).toMatchObject({
      type: "server_error",
      code: "server_is_overloaded",
    });
    expect(
      classifyError(500, "upstream_error", "model is currently overloaded"),
    ).toMatchObject({
      code: "server_is_overloaded",
    });
  });
  ...
  test("transient 429 quota bucket stays retryable (rate_limit_exceeded), delay text preserved", () => {
    const r = classifyError(
      429,
      "upstream_error",
      "You have exceeded your quota for requests per min. Please try again in 5s",
    );

...
describe("provider rail source contract", () => {
  ...
  test("redirects the legacy workspace subroute and normalizes unknown suffixes", async () => {
    const { resolveAppHashChange } = await import("../gui/src/app-routing");
    const legacy = resolveAppHashChange("providers/workspace");
    expect(legacy.route.view).toBe("leveranciers");
    expect(legacy.replaceTo).toBe("leveranciers");
    expect(resolveAppHashChange("providers/other").replaceTo).toBe("leveranciers");
    ...
    expect(routing).toContain('"providers/workspace": "leveranciers"');

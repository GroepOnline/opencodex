...
describe("Claude Desktop profile", () => {
  test("reconciles new routes into Opus with stable unique date aliases", () => {
    ...
    expect(first.assignments["anthropic/claude-fable-5"]?.alias).toBe(
      "claude-fable-5",
    );
    expect(first.assignments["native/gpt-5.6-sol"]?.alias).toMatch(
      /^claude-opus-4-8-2026\d{4}$/,
    );
    expect(
      new Set(Object.values(first.assignments).map((value) => value.alias))
        .size,
    ).toBe(3);
  });
  ...
  test("moves routes and maintains one default per non-empty family", () => {
    ...
    const selected = setDesktopFamilyDefault(
      moved,
      "opus",
      "native/gpt-5.6-sol",
    );
    expect(selected.defaults.opus).toBe("native/gpt-5.6-sol");
    expect(() => setDesktopFamilyDefault(selected, "opus", null)).toThrow(
      DesktopProfileError,
    );
  });
  ...
  test("retains unavailable routes and promotes an active sibling only while rendering", () => {
    ...
    const withoutDefault = renderDesktopProfile(
      profile,
      models.filter((model) => model.route !== "native/gpt-5.6-sol"),
    );
    expect(
      withoutDefault.find((model) => model.family === "opus")?.isFamilyDefault,
    ).toBe(true);
    expect(profile.defaults.opus).toBe("native/gpt-5.6-sol");
    const restored = renderDesktopProfile(profile, models);
    expect(
      restored.find((model) => model.route === "native/gpt-5.6-sol")
        ?.isFamilyDefault,
    ).toBe(true);
  });
  ...
  test("renders family defaults first and only asserts 1M from authoritative metadata", () => {
    ...
    expect(
      rendered.find((model) => model.route === "native/gpt-5.6-sol")
        ?.supports1m,
    ).toBe(true);
    expect(
  ...
  test("rejects unknown fields, duplicate aliases and invalid defaults", () => {
    ...
    expect(() => parseDesktopProfile({ ...profile, extra: true })).toThrow(
      "unknown field",
    );
    const duplicate = structuredClone(profile);
    duplicate.assignments["cursor/gpt-5.6-luna"]!.alias =
      duplicate.assignments["native/gpt-5.6-sol"]!.alias;
  ...
  test("fills all 365 encoded slots then skips overflow without mutating the saved profile", () => {
    ...
    const next = reconcileDesktopProfile(full, [
      ...encoded,
      { route: "test/overflow", label: "Overflow" },
    ]);
    expect(next.assignments["test/overflow"]).toBeUndefined();
    expect(Object.keys(next.assignments)).toHaveLength(365);
    expect(full).toEqual(snapshot);
  });

  test("removeDesktopRoute frees a date slot and repairs the family default", () => {
    const encoded = Array.from({ length: 365 }, (_, index) => ({
      route: `test/model-${index}`,
      label: `Model ${index}`,
    }));
    const full = reconcileDesktopProfile(emptyDesktopProfile(), encoded);
    const removed = removeDesktopRoute(full, "test/model-0");
    expect(removed.assignments["test/model-0"]).toBeUndefined();
    expect(Object.keys(removed.assignments)).toHaveLength(364);
    expect(removed.defaults.opus).not.toBe("test/model-0");
    const next = reconcileDesktopProfile(removed, [
      ...encoded.filter((model) => model.route !== "test/model-0"),
      { route: "mistral/voxtral-mini-tts-latest", label: "Voxtral" },
    ]);
    expect(next.assignments["mistral/voxtral-mini-tts-latest"]?.alias).toMatch(
      /^claude-opus-4-8-2026\d{4}$/,
    );
  });
  ...
  describe("applied-state markers", () => {
    ...
    function seeded() {
      return {
        ...reconcileDesktopProfile(emptyDesktopProfile(), models),
        ...applied,
      };
    }
    ...
    test("reconcileDesktopProfile keeps them across a catalog change", () => {
      const next = reconcileDesktopProfile(seeded(), [
        ...models,
        { route: "test/new-model", label: "New" },
      ]);
    ...
    test("setDesktopFamilyDefault keeps them", () => {
      const moved = moveDesktopRoute(seeded(), "cursor/gpt-5.6-luna", "sonnet");
      const next = setDesktopFamilyDefault(
        moved,
        "sonnet",
        "cursor/gpt-5.6-luna",
      );
    ...
    test("a profile without the markers stays without them", () => {
      const parsed = parseDesktopProfile(
        reconcileDesktopProfile(emptyDesktopProfile(), models),
      );
    ...
    test("non-string markers are rejected with the field named", () => {
      expect(() =>
        parseDesktopProfile({ ...seeded(), appliedFingerprint: 42 }),
      ).toThrow("profile.appliedFingerprint");
      expect(() => parseDesktopProfile({ ...seeded(), appliedAt: {} })).toThrow(
        "profile.appliedAt",
      );
    });
    ...
    test("genuinely unknown fields are still rejected", () => {
      expect(() =>
        parseDesktopProfile({ ...seeded(), bogusField: "x" }),

...
describe("digest deploy workflow contract", () => {
  ...
  test("rollback verifies the exact pre-deploy runtime health contract", async () => {
    ...
    expect(capture).toContain("no discoverable Tailscale IPv4 while capturing pre-deploy health");
    ...
    expect(rollback).toContain('sudo chown -R "$prev_uid:$prev_gid" "$STATE_DIR"');
    expect(rollback).toContain("could not resolve previous container uid/gid for rollback");
